...
int CMemoryView::set_scroll_range()
{
  ...
  int nPos= pDoc->m_uAddress - 0x8000;
  if (nPos > rng - scr)
      nPos = rng - scr;
...
int CMemoryView::find_prev_addr(UINT16 &addr, const COutputMem &mem, int cnt/*= 1*/, int bytes/*= 0*/)
{
  ASSERT(cnt > 0);
  if (cnt < 0)
    return 0;
...
int CMemoryView::find_next_addr(UINT16 &addr, const COutputMem &mem,
...
int CMemoryView::find_delta(UINT16 &addr, UINT16 dest, const COutputMem &mem, int max_lines)
{
  if (dest == addr)
    return 0;
